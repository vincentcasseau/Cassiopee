...
def _convertStruct2NGon(method="geometric", addDataSets=False, api=1):
    caseRootPath = os.path.join(OUTFOLDER, "out_3dSTRUCT")
    C.clearAllNames()
    ...
    C.convertPyTree2File(t, caseRootPath + ".cgns")
    tng = C.convertArray2NGon(t, recoverBC=True, method=method, api=api)
    if method == "geometric":
        C.convertPyTree2File(tng, caseRootPath + "_ng_REF.cgns")
    else:
        C.convertPyTree2File(tng, caseRootPath + "_ng.cgns")
        test.testT(tng, reference=caseRootPath + "_ng_REF.cgns")

def _convertQuad2NGon(method="geometric", addDataSets=False, api=1):
    caseRootPath = os.path.join(OUTFOLDER, "out_2dBE")
    C.clearAllNames()
...
def _convertHexa2NGon(method="geometric", addDataSets=False, api=1):
    caseRootPath = os.path.join(OUTFOLDER, "out_3dBE")
    C.clearAllNames()
...
def _convertME2NGon(method="geometric", addDataSets=False):
    caseRootPath = os.path.join(OUTFOLDER, "out_3dME")
    C.clearAllNames()
