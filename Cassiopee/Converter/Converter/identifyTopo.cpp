...
// Identify face indices topologically
...
PyObject* K_CONVERTER::identifyFacesTopo(PyObject* self, PyObject* args)
{
  ...
  if (res1 != 2 || res2 != 2)
  {
    if (res1 != 2)
      PyErr_SetString(PyExc_TypeError,
                      "identifyTopo: array1 is invalid.");
    if (res2 != 2) 
      PyErr_SetString(PyExc_TypeError,
                      "identifyTopo: array2 is invalid.");
  ...
  if (vertexIndir != NULL)
  {
    ...
    {
      PyErr_SetString(PyExc_TypeError,
                      "identifyTopo: vertex indirection must be a numpy of "
                      "integers.");
  ...
  if (faceIndices != NULL)
  {
    ...
    {
      PyErr_SetString(PyExc_TypeError,
                      "identifyTopo: subset of face indices must be a numpy of "
                      "integers.");
  ...
  {
    PyErr_SetString(PyExc_TypeError,
                    "identifyTopo: BE/ME array1 not implemented yet.");
    RELEASESHAREDU(array1, f1, cn1);
    RELEASESHAREDU(array2, f2, cn2);
    if (vertexIndir != NULL) RELEASESHAREDN(vertexIndir, listVertexIndir);
  ...
  {
    PyErr_SetString(PyExc_TypeError,
                    "identifyTopo: NGON array2 not implemented yet.");
    RELEASESHAREDU(array1, f1, cn1);
    RELEASESHAREDU(array2, f2, cn2);
    if (vertexIndir != NULL) RELEASESHAREDN(vertexIndir, listVertexIndir);
    if (faceIndices != NULL) RELEASESHAREDN(faceIndices, listFaceIndices);
  ...
  else  // BE/ME
  {
    ...
    {
      PyErr_SetString(PyExc_TypeError,
                      "identifyTopo: dimension of array2 must be one less that "
                      "of array1.");
      RELEASESHAREDU(array1, f1, cn1);
      RELEASESHAREDU(array2, f2, cn2);
      if (vertexIndir != NULL) RELEASESHAREDN(vertexIndir, listVertexIndir);
    ...
    // Compute the cumulative number of elts per conn.
      ...
      for (E_Int ic = 0; ic < nc2; ic++)
      {
        ...
        if (vertexIndir == NULL)
        {
          ...
          for (E_Int i = 0; i < nelts; i++)
          {
            ...
            if (resf != faceMap.end()) fmapp[eidx] = resf->second;
            else fmapp[eidx] = -1;
          }
        }
        else
        {
          ...
          for (E_Int i = 0; i < nelts; i++)
          {
            ...
            {
              vidx = cm(i,j+1);
              eltloc[j] = listVertexIndirp[vidx-1] + 1;  // reindex
            }
            F.set(eltloc, nvpe);
            auto resf = faceMap.find(F);
            if (resf != faceMap.end()) fmapp[eidx] = resf->second;
