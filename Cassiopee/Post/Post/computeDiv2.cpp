...
PyObject* K_POST::computeDiv2Struct(PyObject* self, PyObject* args)
{
  ...
  for (E_Int i = 0; i < nfld; i++)
  {
    ...
    char* local;
    computeDivVarsString(vars[i*3], local);
    varStrings.push_back(local);
  }
  ...
  // Build
  E_Int nbIntTot = ni*njc*nkc + nic*nj*nkc + nic*njc*nk;
...
)
{
  ...
  #pragma omp parallel
  {
    ...
    for (E_Int i = 0; i < ni2; i++)
    {
      ...
      d13z = zt[indp] - zt[indm];
      d24x = xt[indm] - xt[indp];
      d24y = yt[indm] - yt[indp];
      d24z = zt[indm] - zt[indp];
  ...
  K_ARRAY::getFromArray3(tpl, gn);
  gn->setAllValuesAtNull();
  E_Float* siint = sint.begin(1);
  E_Float* sjint = sint.begin(2);
  E_Float* skint = sint.begin(3);
  ...
  for (E_Int n = 0; n < nfld; n++)
  {
    ...
    for (E_Int i = 0; i < nbIntTot; i++)
    {
      ...
      if (indcellg != -1)
      {
        gpdv[indcellg] += ffi*siint[i] + ffj*sjint[i] + ffk*skint[i];
      }
      if (indcelld != -1)
      {
        gpdv[indcelld] -= ffi*siint[i] + ffj*sjint[i] + ffk*skint[i];
      }
    }
  }
  // free mem
