    ...
    for trip in range(Cmpi.size-1):
        ...
        if zu is not None:
            ...
            if sizebc > 0:
                ...
                #print(f"[{Cmpi.rank}] {trip}: Creating match {zu[0]} for {z[0]}", flush=True)
                mask2 = ~mask
                indices = numpy.nonzero(mask2)[0].astype(Internal.E_NpyInt)
                #print(f"[{Cmpi.rank}] {trip}: indices.size = {indices.size}", flush=True)
                if indices.size:
                    zoneName = zu[0]
        ...
        if data is not None:
            ...
            if zoneDonor not in neiRankNames:
                C._addBC2Zone(z, 'match', 'BCMatch', faceList=faceList,
                              zoneDonor=zoneDonor, faceListDonor=faceListDonor)
                neiRankNames.add(zoneDonor)
                print(f"[{Cmpi.rank}] {trip}: Mirroring match {zoneDonor} for {z[0]}", flush=True)
            else: data = None
        ...
        if zu is not None:
            ...
            if sizebc > 0:
                mask2 = ~mask
                if data is not None:
                    print(f"[{Cmpi.rank}] {trip}: H1 = {indicesE}, {faceList}, {faceListDonor}", flush=True)
                    mask2 &= ~numpy.isin(indicesE, faceList)
            elif data is not None:
                print(f"[{Cmpi.rank}] {trip}: H2 = {indicesE}, {faceList}, {faceListDonor}", flush=True)
                mask2 = ~numpy.isin(indicesE, faceList)
